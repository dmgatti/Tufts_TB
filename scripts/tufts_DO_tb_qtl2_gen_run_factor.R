...
result_dir = file.path(base_dir, 'results', 'qtl2', 'gen_run')
...
covar$aero_run[covar$gen == 'G21'] = 1
...
# Permutations.
nperm = 1000
perms = matrix(0, nrow = nperm, ncol = ncol(pheno), 
               dimnames = list(1:nperm, colnames(pheno)))
...
for(i in 15:ncol(pheno)) {
  ...
  perms[,i] = qtl2::scan1perm(genoprobs = probs[samples2use,],
                          pheno   = pheno[samples2use,i,drop = FALSE], 
                          kinship = tmpK, 
                          addcovar = tmp_covar, 
                          cores  = 5,
                          n_perm = nperm,
...
qtl = qtl[,cl$order]
markers = unlist(map)
markers = data.frame(one = names(markers), pos = markers) %>%
             separate(one, into = c('chr', 'marker'), sep = '\\.') %>%
             select(marker, chr, pos)
rownames(markers) = markers$marker
markers$chr = factor(markers$chr, levels = c(1:19, 'X'))

qtl = merge(markers, qtl, by = 'row.names')
rownames(qtl) = qtl$Row.names
qtl = qtl[,-1]
...
qtl_heatmap = function(lod) {
  ...
  new_qtl %>%
    ...
    ggplot() +
      ...
      scale_color_gradientn(colors = colors, values = scales::rescale(c(0, 4.0, 6.0, 7.0, 8.0, max(lod) + 0.1), to = c(0,1))) +
      scale_fill_gradientn(colors  = colors, values = scales::rescale(c(0, 4.0, 6.0, 7.0, 8.0, max(lod) + 0.1), to = c(0,1))) +
      facet_grid(pheno ~ chr, scales = 'free_x') +
