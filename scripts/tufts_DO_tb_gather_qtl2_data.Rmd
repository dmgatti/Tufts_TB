...
probs_file  = file.path(base_dir, 'haplo_reconstr', 'tufts_do_alleleprobs_cleaned_20210601.rds')
...
```{r make_pheno_covar}
covar = select(pheno, mouse:map_lung)
rownames(covar) = covar$mouse
pheno = select(pheno, mouse, pct_wt_loss:serum_cxcl1)
```
...
```{r make_kinship}
K = calc_kinship(probs, type = 'loco', cores = 8)
map = map[c(1:19, 'X')]
```
